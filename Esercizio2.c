...
clock_t doExperiment(int* randomArray, const unsigned int numInsertions, const unsigned int numSearches, char* dataStructure) {
    ...
    int key;
    hashtable_t* table = createHashtable(NUM_ENTRIES);
    linkedListNode_t* nod_hash;
    rbtNode_t* nod_rbt;
    rbt_t* rbt= createRbt();
    ...
    if(strcmp(dataStructure, "hashtable")==0){
        ...
        for(int j=0; j<numSearches; j++){
            key=rand()% MAX_RANDOM_NUMBER +1;
            nod_hash=hashtableSearch(table,key);
        }
    }
    ...
    endTime=clock();
    hashtableFree(table); // devo cancellare prima le liste
   
    rbtFree(rbt); // devo cancellare prima i nodi
